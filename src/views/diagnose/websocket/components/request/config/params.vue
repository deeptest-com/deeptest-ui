<template>
  <div class="request-params-main">
    <div class="dp-param-grid">
      ...
      <div class="params">
        <a-row v-for="(item, idx) in model.params" :key="idx" type="flex" class="param">
          <a-col flex="1">
...
const add = () => {
  console.log('add')
  model.value.params.push({} as Param)
}
const removeAll = () => {
  console.log('removeAll')
  model.value.params = [{} as Param]
}
const insert = (idx) => {
  console.log('insert')
  model.value.params.splice(idx+1, 0, {} as Param)
}
const disable = (idx) => {
  console.log('enable', idx)
  model.value.params[idx].disabled = !model.value.params[idx].disabled
}
const remove = (idx) => {
  console.log('remove')
  model.value.params.splice(idx, 1)
  const len = model.value.params.length
  if (len == 0 || !!model.value.params[len-1].name) {
    add()
  }
}
...
const onParamChange = (idx) => {
  ...
  if (model.value.params.length <= idx + 1
      && (model.value.params[idx].name !== '' || model.value.params[idx].value !== '')) {
    model.value.params.push({} as Param)
  }
};
