...
import {listExtractorVariable} from "@/views/component/debug/service";
...
const model = computed<any>(() => store.state.Endpoint.activeAlternativeCaseAssertion);
const baseCaseId = computed<any>(() => store.state.Endpoint.baseCaseId);
...
const save = () => {
  ...
  validate().then(() => {
    store.dispatch('Endpoint/saveAlternativeCaseAssertion', model.value).then((result) => {
      if (result) {
        notifySuccess(`保存成功`);
        if (props.finish) {
          props.finish()
...
const loadExtractorVariable = () => {
  ...
  if (model.value.type === CheckpointType.extractor) {
    listExtractorVariable({debugInterfaceId: model.value.debugInterfaceId}).then((jsn) => {
