<template>
  <div class="js-lib-list-main">
    <a-card :bordered="false">
      <template #title>
        <a-button type="primary" @click="() => edit(0)">新建工具大模型</a-button>
      </template>
      <template #extra>
        <a-input-search
            @change="onSearch"
      ...
      <EmptyComp>
        <template #content>
          <a-table class="dp-table"
                    :data-source="models" :columns="llmColumns" :rowKey="(_record, index) => _record.id"
                   :pagination="{
                      showTotal: (total) => {
                         return `共 ${total} 条数据`;
                      },
            ...
            <template #name="{ text, record }">
              <div class="record-name">
                <EditAndShowField :custom-class="'custom-serve show-on-hover'" placeholder="请输入工具大模型名称"
                                  :value="text || ''"
            ...
            <template #setDefault="{ record }">
              <a-radio v-model:checked="record.isDefault"
...
const models = computed<any>(() => store.state.ProjectSetting.llmModels);
...
function list() {
  ...
  store.dispatch('ProjectSetting/listLlm', queryParams.value)
}

function updateName(value: string, record: any) {
  store.dispatch('ProjectSetting/updateLlmName', {
    id: record.id,
    name: value
  });
}
...
const setDefault = (record) => {
  store.dispatch('ProjectSetting/setDefaultLlm', record.id);
}

async function onDisable(record: any) {
  store.dispatch('ProjectSetting/disableLlm', record.id);
}
