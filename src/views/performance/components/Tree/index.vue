...
import {StateType as PerformanceStateType} from "../store";
import {filterTree, filterByKeyword} from "@/utils/tree";
import {notifyError, notifySuccess, notifyWarn} from "@/utils/notify";
import { DropdownActionMenu } from '@/components/DropDownMenu';

const store = useStore<{ Performance: PerformanceStateType, ProjectGlobal: ProjectStateType }>();
const currProject = computed<any>(() => store.state.ProjectGlobal.currProject);
const treeDataCategory = computed<any>(() => store.state.Performance.treeDataCategory);
const treeDataMapCategory = computed<any>(() => store.state.Performance.treeDataMapCategory);
...
async function loadCategories() {
  await store.dispatch('Performance/loadCategory');
  //expandAll();
}
...
watch(() => {
  ...
  if (newVal?.id && newVal?.id !== oldVal?.id) {
    store.commit('Performance/setTreeDataCategory', {});
    searchValue.value = '';
...
async function selectTreeItem(keys, e) {
  ...
  // 前端缓存选中的节点
  await setSelectedKey('category-performance-testplan', currProject.value.id, selectedKeys.value[0])

  // 如果没有选中的节点，就默认选中根节点
  emit('select', nodeVal ? selectedKeys.value[0] : null);
  const selectedData = treeDataMapCategory.value[selectedKeys.value[0]]

  // 选中节点后，需要将选中节点的数据存入Store
  if(nodeVal !== -1){
    console.log("select node",selectedData)
    await store.commit('Performance/setNodeCategory',selectedData);
  } else {
    await store.commit('Performance/setNodeCategory', {id: -1, name: '未分类', children: []});
  }
}
...
async function deleteCategory(node) {
  Modal.confirm({
    ...
    cancelText: () => '取消',
    onOk: async () => {
      const res = await store.dispatch('Performance/removeCategoryNode', {
        id:node.id,
        type:'performance'
      });
      if (res) {
...
async function handleTagModalOk(obj) {
  console.log('handleTagModalOk')

  obj = Object.assign(currentNode.value, obj);

  if (tagModalMode.value === 'edit') { // 修改
    const res = await store.dispatch('Performance/updateCategoryNode', {
      id: obj.id,
      name: obj.name,
      type: "performance-testplan",
      desc: obj.desc,
    });
    if (res) {
    ...
    const res = await store.dispatch('Performance/createCategoryNode', {
      "name": obj.name,
      "desc": obj.desc,
      "mode": "child",
      "targetId": obj.id,
...
async function onDrop(info: DropEvent) {
  ...
  const res = await store.dispatch('Performance/moveCategoryNode', {
